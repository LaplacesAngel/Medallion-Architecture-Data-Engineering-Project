...
from pyspark.sql.functions import count, year, col
...
silver_schema = StructType( [ StructField("customer_id", StringType(), nullable=True),
...
silver_data =   spark \
                ...
                .parquet("s3a://hwe-fall-2023/plentz/silver/reviews/")
...
aggregated_data =   watermarked_data \
                    .groupBy("city", "state", "review_timestamp", "date_of_birth", "gender", "helpful_votes", "purchase_date") \
                    .avg("star_rating") \
                    .withColumnRenamed("avg(star_rating)", "average_star_rating") \

# aggregated_data =   watermarked_data \
#                     .groupBy("gender", "review_timestamp") \
#                     .sum("helpful_votes") \
#                     .withColumnRenamed("sum(helpful_votes)", "total_helpful_votes") \
...
write_gold_query =  aggregated_data \
                    ...
                    .outputMode("append") \
                    .format("delta") \
                    .option("path", "s3a://hwe-fall-2023/plentz/gold/fact_review") \
